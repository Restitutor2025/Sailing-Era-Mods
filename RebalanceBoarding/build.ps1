# Builds Restitutor_Rebalance_Boarding.dll, runs checks against the installed GameAssembly, installs to Mods\Rebalance.
...
dotnet build "$here\Restitutor_Rebalance_Boarding.csproj" -c Release -p:NuGetAudit=false
if($LASTEXITCODE -ne 0){ Write-Host "build failed ($LASTEXITCODE)"; exit 1 }
dotnet run --project "$here\tests\Tests.csproj" -c Release -- "$game\GameAssembly.dll"
if($LASTEXITCODE -ne 0){ Write-Host "tests failed ($LASTEXITCODE)"; exit 1 }
$dll="$here\bin\Release\net6.0\Restitutor_Rebalance_Boarding.dll"
...
Get-FileHash "$out\Restitutor_Rebalance_Boarding.dll","$dest\Restitutor_Rebalance_Boarding.dll" -Algorithm SHA256 | Format-List
