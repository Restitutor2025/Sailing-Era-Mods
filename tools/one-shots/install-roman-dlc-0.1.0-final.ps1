...
$coreHash='5199A1318118D79507046472A7C4C419353C06CF06B3348551EFE4396865E3B2'
$dlcHash='4D99517FB8E6E4A879FDB0F3A9538EEEE0114729DD8C50DD9989DFBF10756649'
$oldHash='9BADB30CC0B3E6B63C9DC0F87FFB7901601B8E794BE073E7531C9D5F2FC0BD8A'
$targets=@(
...
foreach ($t in $targets) {
  ...
  if (Test-Path -LiteralPath $t.target) {
    ...
    if ($t.before -ne $t.hash -and (($t.name -eq 'dlc' -and $t.before -ne '8F5BC746DD57DF3B97F2E5BD4A92441ACF67F0DB4C3D0C407FFC3860E1F8C698') -or ($t.name -ne 'dlc' -and $t.before -ne $oldHash))) { throw ('Unexpected installed hash: '+$t.target) }
  } else { $t.before=$null }
}
