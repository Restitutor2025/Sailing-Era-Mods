# fix-contribution-0.5.7: Contribution 0.5.7 - restores the UI artwork missing from the 0.5.6 build (city benefit HUD), adds assets to the repo.
# Run with the game closed. Safe to run again. Roll back: Contribution\releases\0.5.6 is the broken build; use 0.5.7.
...
git merge-base --is-ancestor 'refactor/cheats-core' origin/main; if($LASTEXITCODE -ne 0){ Fail '이전 PR(refactor/cheats-core)이 아직 병합되지 않았습니다. GitHub 에서 먼저 Merge 후 다시 실행' }
$jobs=@(
 @{src="$p\Contribution\releases\0.5.7\Restitutor_fixes_Contribution.dll"; dst="$g\Mods\Additional_Functions\Restitutor_Additional_Contribution_Goods_Recognition.dll"; want='3DD26315F1DBDCAA48F20BC9E69FBFDAE4D8609C56A718095196BD841BD94F43'})
...
git switch -q 'fix/contribution-assets' 2>$null; if($LASTEXITCODE -ne 0){ git switch -q -c 'fix/contribution-assets' origin/main; if($LASTEXITCODE -ne 0){ Fail 'git switch' } }
git branch -f main origin/main 2>$null | Out-Null
Add @('Contribution','docs/mods/contribution','.gitignore'); Commit 'fix(contribution): ship embedded UI artwork again (0.5.7)' '0.5.6 was built from a working copy without assets/*.png, so the EmbeddedResource items were missing (Missing UI artwork: city-panel). Code unchanged; assets are now tracked in the repo.'
Add @('tools','fix-contribution-0.5.7.ps1','fix-contribution-0.5.7.bat'); Commit 'chore: Contribution 0.5.7 install script' ''
git push -q -u origin 'fix/contribution-assets'; if($LASTEXITCODE -ne 0){ Fail 'git push' }
git log --oneline origin/main..HEAD
$pr='https://github.com/Restitutor2025/Sailing-Era-Mods/compare/main...fix/contribution-assets?expand=1'
